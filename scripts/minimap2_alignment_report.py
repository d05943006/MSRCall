    ...
    for record in allrecords:
        if record.flag != 4:
            ...
            print(record.Qname, '\t', record.Rname, '\t', record.map_quality, '\t', record.flag, '\t', record.qs, '\t',
                  ...
                  1 - record.dle * 1.0 / (record.re - record.rs) - record.mis * 1.0 / (record.re - record.rs), '\t', # Adam: why is this identity rate
                  record.mis * 1.0 / (record.re - record.rs) + record.ins * 1.0 / (
                              record.re - record.rs) + record.dle * 1.0 / (record.re - record.rs), '\t', record.map_len)
            '''
    deletion_rate  = 1.0 * nDel / totalLen
    insertion_rate = 1.0 * nIns / totalLen
    mismatch_rate  = 1.0 * nmiS / totalLen
    print("total file, deletion, instertion, mismatch rate")
    print('{} \t{:.5f} \t{:.5f} \t# {:.5f}'.format(nFile, deletion_rate, insertion_rate, mismatch_rate)) # Adam added
